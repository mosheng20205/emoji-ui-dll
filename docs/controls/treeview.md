...
' 创建树形框
树形框句柄 = CreateTreeView (窗口句柄, 20, 20, 450, 550, #FFFFFFFF, #FF000000, 0)
...
' 设置回调函数
SetTreeViewCallback (树形框句柄, 1, &节点选中回调)
SetTreeViewCallback (树形框句柄, 2, &节点展开回调)
SetTreeViewCallback (树形框句柄, 7, &节点复选框回调)
...
文本 = 到UTF8 ("📁 我的文档")
图标 = 到UTF8 ("📁")
根节点1 = AddRootNode (树形框句柄, 文本, 取字节集长度 (文本), 图标, 取字节集长度 (图标))

' 添加子节点 - Word 文档
文本 = 到UTF8 ("📄 工作报告.docx")
图标 = 到UTF8 ("📄")
子节点1 = AddChildNode (树形框句柄, 根节点1, 文本, 取字节集长度 (文本), 图标, 取字节集长度 (图标))

' 设置复选框
SetNodeCheckBox (树形框句柄, 根节点1, 真)
SetNodeCheckBox (树形框句柄, 子节点1, 真)

' 展开根节点
ExpandNode (树形框句柄, 根节点1)
...
父节点ID = GetNodeParent (树形框句柄, 节点ID)
层级 = GetNodeLevel (树形框句柄, 节点ID)
子节点数 = GetNodeChildCount (树形框句柄, 节点ID)
...
EnableTreeViewDragDrop (树形框句柄, 真)
调试输出 ("启用拖放功能")
...
文本 = 到UTF8 ("工作报告.docx")
查找结果 = FindNodeByText (树形框句柄, 文本, 取字节集长度 (文本))

如果 (查找结果 > 0)
    调试输出 ("找到节点: ID = " + 到文本 (查找结果))
    SetSelectedNode (树形框句柄, 查找结果)
    ScrollToNode (树形框句柄, 查找结果)
否则
    调试输出 ("未找到节点")
如果结束
...
```
' 设置重要节点为红色
SetNodeForeColor (树形框句柄, 节点ID, #FFFF0000)

' 设置选中节点为黄色背景
SetNodeBackColor (树形框句柄, 节点ID, #FFFFFF00)

' 禁用某些节点
SetNodeEnabled (树形框句柄, 节点ID, 假)
```
...
- 避免频繁调用 `ExpandAll()`，可能导致性能问题
- 使用 `FindNodeByText()` 前先缓存节点ID
- 拖放功能会增加内存占用，不需要时应禁用
